...
class Tarjeta_Regalo_Directa(unittest.TestCase):
    def setUp(self):
        try:
            ...
            logging.basicConfig(filename='log/'+'Sanity_011_log_Tarjeta_Regalo_Directa', filemode='w', encoding='utf-8', level=logging.INFO, format='[%(asctime)s] [%(levelname)-8s] %(message)s',datefmt='%Y-%m-%d %H:%M:%S')
